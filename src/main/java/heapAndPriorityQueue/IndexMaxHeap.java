...
public class IndexMaxHeap<Item extends Comparable> {
    ...
    protected int[] indexs;   // 最大索引堆中的索引, indexes[x] = i 表示索引i在x的位置
    ...
    private void shiftDown(int k){
        while (2*k <= count){
            int j = 2*k;
            if(j+1<count && data[indexs[j]].compareTo(data[indexs[j+1]]) < 0)
                j++;
            if(data[indexs[k]].compareTo(data[indexs[j]]) >= 0) break;
            swapIndex(j,k );
            k = j;
        }
    }
